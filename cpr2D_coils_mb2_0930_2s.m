function [Phase_coeff,PhaseMap_pc] = cpr2D_coils_mb2_0930_2s(kspace,Image_ref,CSP_MB,CSP_SB,PhaseMap_1D,FOVshift,nshot,weight_csp)
...
Image_ref_s0 = squeeze(Image_ref(:,:,1,:));
Image_ref_s1 = squeeze(Image_ref(:,:,2,:));
...
[Mask_s1,~] = Mask_Generate(RMS_tmp1,0.16,0.16);
if FOVshift == 1/2
...
tmp = Mask_s0 + Mask_s1;
tmp1 = tmp > 0 ;
a1 = tmp1 + Mask_s0([Ny_MB/4+1:end 1:Ny_MB/4],:);
a1 = a1 > 1;
a2 = tmp1 + Mask_s0([Ny_MB*3/4+1:end 1:Ny_MB*3/4],:);
a2 = a2 >1;

a3 = tmp1 + Mask_s1([Ny_MB/4+1:end 1:Ny_MB/4],:);
a3 = a3 > 1;
a4 = tmp1 + Mask_s1([Ny_MB*3/4+1:end 1:Ny_MB*3/4],:);
a4 = a4 >1;
...
% Mask_background = zeros(Ny_MB,Nx_MB);
...
FOV = Ny_MB;
Phase_coeff = zeros(Nx_MB,2);
coeff1step_c1 = 0.02;
coeff2step_c2 = 0.002;
savefactor1 = 0.05;
savefactor2 = 0.005;

Difference = zeros(Nx_MB,2,2);
ImageCorr = zeros(Ny_MB,Nx_MB);
num_search = 1;
Index_csp = zeros(Nx_MB,2,2);
for k1 = xlocation0 : xend
    if k1 == xlocation0
        coeff1list = round(-savefactor1*pi,1) : coeff1step_c1 : round(savefactor1*pi,1);
        ...
        coeff1list = ref1 - num_search*coeff1step_c1 : coeff1step_c1 : ref1 + num_search*coeff1step_c1;
        coeff1ln = length(coeff1list);
        coeff2list = ref2 - num_search*coeff2step_c2 : coeff2step_c2 : ref2 + num_search*coeff2step_c2;
        coeff2ln = length(coeff2list);
    end
    
    LineCorr = zeros(Ny_MB,coeff1ln,coeff2ln,Ncoil_MB);
    DiffSum_csp = zeros(coeff1ln,coeff2ln,Ncoil_MB);
    DiffSum_bg = zeros(coeff1ln,coeff2ln);
    LineCorrCombine = zeros(Ny_MB,coeff1ln,coeff2ln);
    CoilSensProf_Ite = zeros(Ny_MB,coeff1ln,coeff2ln,Ncoil_MB);
    for C1 =  1 : coeff1ln 
        for C2 = 1 : coeff2ln 
            
            pc1 = coeff1list(C1);
            pc2 = transpose(-Ny_MB/2 : Ny_MB/2-1);
            pc2 = coeff2list(C2)*pc2;
            pc = pc1 + pc2 + PhaseMap_1D(:,k1) ;
            
            CSP_matrix =  zeros(ns4*Ncoil_MB*Ny_MB/ns4,Ny_MB*Ncoil_MB); % csp and phase matrix
            ...
            for k2 = 1 : Ny_MB/ns4  % y direction
                ...
                for c = 1 : Ncoil_MB
                    CSP_matrix((k2-1)*Ncoil_MB*ns4 + (c-1)*ns4+1,(k2-1)*Ncoil_MB*ns4 + (c-1)*4+1:(k2-1)*Ncoil_MB*ns4 + 4*c) = 1/ns4.*Pos_k1.*pc_r;
                    CSP_matrix((k2-1)*Ncoil_MB*ns4 + (c-1)*ns4+2,(k2-1)*Ncoil_MB*ns4 + (c-1)*4+1:(k2-1)*Ncoil_MB*ns4 + 4*c) = 1/ns4.*Pos_k2.*pc_r;
                    
                    CSP_matrix((k2-1)*Ncoil_MB*ns4 + (c-1)*ns4+3,(k2-1)*Ncoil_MB*ns4 + (c-1)*4+1:(k2-1)*Ncoil_MB*ns4 + 4*c) = 1/ns4.*Neg_k3;
                    CSP_matrix((k2-1)*Ncoil_MB*ns4 + (c-1)*ns4+4,(k2-1)*Ncoil_MB*ns4 + (c-1)*4+1:(k2-1)*Ncoil_MB*ns4 + 4*c) = 1/ns4.*Neg_k4;
            ...
            for k2 = 1 : Ny_MB/ns4
                ...
                p(p>FOV) = p(p>FOV) - FOV;
                for c = 1 : Ncoil_MB
                    LineCorr(p,C1,C2,c) = S((k2-1)*Ncoil_MB*ns4 + (c-1)*ns4 + 1 : (k2-1)*Ncoil_MB*ns4 + (c-1)*ns4 + 4);
                end
            end
            ...
            CoilSensProf_Ite(:,C1,C2,:) = CoilSensProf_Ite(:,C1,C2,:)./CoilSensProf_Ite(:,C1,C2,1).*abs(CoilSensProf_Ite(:,C1,C2,1));
            
            for c = 1 : Ncoil_MB
                q1_s1 = CoilSensProf_Ite(:,C1,C2,c).*Mask_csp(:,k1);
                q2_s1 = CSP_MB(:,k1,c).*Mask_csp(:,k1);
    ...
    if sum(Mask_csp(:,k1)) ~= 0
        ...
        if mskld_bg(k1) ~= 0 % csp = 0, bg~= 0
            ...
            Phase_coeff(k1,:,2) = [coeff1list(p1_bg) coeff2list(p2_bg)];
            Phase_coeff(k1,:,3) = [coeff1list(p1_bg) coeff2list(p2_bg)];
            Difference(k1,1,:) = [0 0];
            Difference(k1,2,:) = [min(DiffSum_bg(:)) max(DiffSum_bg(:))];
        end        
    end
...
PhaseMap = zeros(Ny_MB,Nx_MB);
for k1 = xstart : xend
    PhaseMap(:,k1) = exp(1i.*Phase_coeff(k1,2,3).*transpose(-Nx_MB/2:Nx_MB/2-1)).*exp(1i*repmat(Phase_coeff(k1,1,3),Ny_MB,1));
end
...
xp = repmat((1:Ny_MB)',length(xstart:xend),1);
yp = reshape(repmat(xstart:xend,Ny_MB,1),[],1);
